...
`endpoint`、`apiKey` 和 `model` 继续通过 URL 参数传递，其优先级高于内嵌配置。未传 `apiKey` 时，可从 JSON 中的 `auth.OPENAI_API_KEY` 或选中供应商的 `experimental_bearer_token` 提取密钥。若 `requires_openai_auth = false`，导入器会将合并后的密钥写入供应商的 `experimental_bearer_token`，确保认证仍然有效。最终供应商标识仍为 `custom`，显示名称来自 URL 的 `name`。
...
此功能需要包含 Codex 供应商选项导入支持的 CC Switch 构建。v3.12.1、v3.20.0 等旧版只提取基础字段，会丢弃这些选项。示例请求头用于客户端兼容配置，实际生图能力仍取决于 Codex 客户端和供应商 API。
