...
Codex 供应商导入可以保留内嵌配置中 `model_provider` 指定的供应商的 `http_headers` 和 `requires_openai_auth`。使用 `configFormat=json`，将以下 JSON 按 UTF-8 编码后再转为 Base64，放入 `config` 参数：
...
`endpoint`、`apiKey` 和 `model` 继续通过 URL 参数传递，其优先级高于内嵌配置。未传 `apiKey` 时，可从 JSON 中的 `auth.OPENAI_API_KEY` 或选中供应商的 `experimental_bearer_token` 提取密钥。若 `requires_openai_auth = false`，导入器会将合并后的密钥写入供应商的 `experimental_bearer_token`，确保认证仍然有效。最终供应商标识仍为 `custom`，显示名称来自 URL 的 `name`。启用供应商时，CC Switch 会按“保留官方登录”设置调整实际配置中的 `requires_openai_auth`；供应商的 Bearer token 和导入的请求头仍会保留。

除现有密钥、地址和模型字段外，只保留上述两项供应商配置；不会导入其他供应商、任意顶层选项、MCP 服务、hooks 或从环境变量读取的请求头。JSON 不是对象、TOML 格式错误、找不到选中的自定义供应商、认证开关不是布尔值或请求头非法时会拒绝导入。未提供新增选项的链接保持原有默认行为。`openai` 等内建供应商无需显式提供供应商表。

此功能需要包含 Codex 供应商选项导入支持的 CC Switch 构建。v3.12.1、v3.20.2 等旧版只提取基础字段，会丢弃这些选项。示例请求头用于客户端兼容配置，实际生图能力仍取决于 Codex 客户端和供应商 API。
