...
### Codex provider HTTP headers and authentication

Codex provider imports can preserve `http_headers` and `requires_openai_auth` from the provider selected by `model_provider` inside an inline config. Use `configFormat=json` and encode the UTF-8 JSON envelope as Base64 in `config`:
...
Continue passing `endpoint`, `apiKey`, and `model` as URL parameters; they take priority over values in the config. The optional JSON `auth.OPENAI_API_KEY` or the selected provider's `experimental_bearer_token` can supply the key when `apiKey` is absent. When `requires_openai_auth = false`, the imported provider's `experimental_bearer_token` is generated from this merged key so authentication still works. The stored provider key remains `custom`; the URL `name` is its display name. On activation, CC Switch aligns the live `requires_openai_auth` flag with its official-login-preservation setting; the provider bearer token and imported HTTP headers remain available.

Only these two provider options are preserved in addition to the existing key, endpoint and model extraction. Other providers, arbitrary top-level options, MCP servers, hooks and environment-variable header references are not imported. Invalid JSON envelopes or TOML, missing selected custom providers, non-boolean auth flags and invalid HTTP headers are rejected. Omitting the new options retains the existing import defaults. Built-in provider IDs such as `openai` do not need an explicit provider table.

This requires a CC Switch build containing Codex provider-option import support. Older releases, including v3.12.1 and v3.20.2, only extract the basic fields and discard these options. The header example supplies client compatibility configuration; actual image support still depends on the Codex client and the provider API.
